...
def create_shared_evaluation_dataset(
    ...
    # Use None/empty string for normal windows (no fault), not "unknown"
    ...
    for i in range(len(X_normalized)):
        if window_labels_binary_check[i] > 0:
            faulty_sensors = [
                SENSOR_COLS_AVAILABLE[j]
                for j in range(len(SENSOR_COLS_AVAILABLE))
                if sensor_labels[i, j] > 0
            ]
    ...
    if not skip_neo4j and gdn_model_path is not None:
        ...
        try:
            ...
            if not model_path.exists():
                    ...
                    if "sensor_embeddings" in checkpoint:
                        ...
                        for key in checkpoint.keys():
                            if "sensor_embeddings" in key:
                                detected_embed_dim = (
                                    checkpoint[key].shape[1]
                                    if hasattr(checkpoint[key], "shape")
                                    else 64
                                )
                                break
                        else:
                            detected_embed_dim = 64
                except:
                    detected_embed_dim = 64
                ...
                # Use GDN predictions (not ground truth labels) for KG construction
                # Keep sensor_labels separate for evaluation purposes only
                kg.construct(
                    X_windows=kg_data["X_windows"],
                    gdn_predictions=kg_data[
                        "gdn_predictions"
                    ],  # GDN predictions, not ground truth
                    X_windows_unnormalized=X_unnormalized,
                )

                print(
                    f"  ✓ Knowledge Graph built (nodes: {kg.kg.number_of_nodes()}, edges: {kg.kg.number_of_edges()})"
                )
                ...
                try:
                    ...
                    # Optionally load only a subset of windows for faster evaluation
                    loader.load_from_kg(
                        kg,
