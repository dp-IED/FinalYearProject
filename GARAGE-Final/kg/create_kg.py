...
def compute_adjacency_matrix(sensor_embeddings: np.ndarray) -> np.ndarray:
    ...
    embeddings_norm = F.normalize(torch.from_numpy(sensor_embeddings), p=2, dim=1).numpy()
    
    # Compute cosine similarity matrix
    similarity_matrix = np.dot(embeddings_norm, embeddings_norm.T)  # (num_sensors, num_sensors)
...
def extract_window_embeddings(model: GDN, X_windows: np.ndarray, batch_size: int = 32, device: str = 'cpu') -> np.ndarray:
    """
...
class KnowledgeGraph:
    ...
    def _process_window(self, window_idx: int, window_data: np.ndarray,
        ...
        for sensor_idx, sensor_name in enumerate(self.sensor_names):
            ...
            stats = WindowStats(
                ...
                trend=float(np.polyfit(np.arange(len(sensor_values)), sensor_values, 1)[0]) if len(sensor_values) > 1 else 0.0,
                median=float(np.median(sensor_values)),
                q25=float(np.percentile(sensor_values, 25)),
                q75=float(np.percentile(sensor_values, 75)),
                variation_from_normal=self._compute_variation_from_normal(sensor_name, sensor_values),
    ...
    def _compute_variation_from_normal(self, sensor_name: str, sensor_values: np.ndarray) -> float:
    ...
    def _compute_distribution_thresholds(self, gdn_predictions: np.ndarray,
                                         sensor_labels_true: Optional[np.ndarray] = None,
        ...
        for i, sensor_i in enumerate(self.sensor_names):
            for j, sensor_j in enumerate(self.sensor_names):
                ...
                if edge_attrs.get('window_idx') == window_idx:
                    ...
                    edge_attrs['violates_gdn_expectation'] = deviation_from_gdn > deviation_threshold
    ...
    def get_window_kg(self, window_idx: int, temporal_context_windows: int = 2) -> Dict[str, Any]:
        """
        ...
        prediction_threshold = 0.5
        anomalous_sensors = {
            sensor_name for sensor_name, stat in window_stats.items()
            if stat.anomaly_score > prediction_threshold
        }
        
        for u, v, data in window_graph.edges(data=True):
            correlation_strength = data.get("correlation_strength", abs(data.get("correlation", 0)))
            is_violation = (data.get("violates_domain_expectation", False) or 
                          data.get("violates_gdn_expectation", False))
            ...
            if not (is_violation or involves_anomaly or is_significant or potential_fault):
                continue
            ...
            relationship = {
                ...
                "expected_correlation_gdn": float(data.get("expected_correlation_gdn", 0)),
                "deviation_from_gdn": float(data.get("deviation_from_gdn", 0)),
                "violates_domain_expectation": data.get("violates_domain_expectation", False),
    ...
    def save(self, path: str):
        ...
        if path.suffix == '.pkl':
            ...
            data = {
                ...
                'window_graphs': {str(k): nx.node_link_data(v) for k, v in self.window_graphs.items()},
                'window_stats': {str(k): {name: asdict(stats) for name, stats in v.items()} 
...
def load_and_preprocess_data(data_path: str, sensor_cols: List[str], 
                             window_size: int = 300) -> Tuple[np.ndarray, np.ndarray]:
    """
