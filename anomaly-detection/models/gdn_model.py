...
class MultiLabelGDN(nn.Module):
    ...
    def get_embeddings(self, x):
        ...
        if B > 0:
            emb_before_norm = torch.norm(embeddings[0]).item()
            with open(
                "/Users/darenpalmer/Desktop/UCL/CS/fyp.nosync/.cursor/debug.log", "a"
            ) as f:
                f.write(
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "A",
                            "location": "gdn_model.py:153",
                            "message": "Window embedding before normalization",
                            "data": {"norm": emb_before_norm},
        ...
        # #region agent log
        if B > 0:
            emb_after_norm = torch.norm(embeddings[0]).item()
            with open(
                "/Users/darenpalmer/Desktop/UCL/CS/fyp.nosync/.cursor/debug.log", "a"
            ) as f:
                f.write(
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "A",
                            "location": "gdn_model.py:156",
                            "message": "Window embedding after normalization",
                            "data": {"norm": emb_after_norm},
                            "timestamp": int(__import__("time").time() * 1000),
                        }
                    )
                    + "\n"
...
class KAGOptimizedGDN(nn.Module):
    ...
    Expected: 80-85% KAG diagnostic quality in 6-8 hours (vs 25+ hours for enhanced)
    """
...
class TripletLoss(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        anchor_idx = torch.randint(
            0, len(normal_emb), (len(normal_emb),), device=embeddings.device
        )
        ...
        neg_distances = torch.cdist(
            anchor.unsqueeze(0), anomalous_emb.unsqueeze(0)
        ).squeeze(0)  # (N, A)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        pair_mask = torch.triu(
            torch.ones(B, B, device=embeddings.device), diagonal=1
        ).bool()
        ...
        neg_loss = (1 - same_class_pairs) * F.relu(
            self.margin - pair_distances
        ) ** 2  # Push dissimilar apart
