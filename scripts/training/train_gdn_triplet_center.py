#!/usr/bin/env python3
"""
Training script for MultiLabelGDN with Triplet-Center Loss and Supervised Contrastive Loss.

Implements improved metric learning techniques:
- Triplet-Center Loss: Combines center pull with triplet margin
- Supervised Contrastive Loss: More robust contrastive learning
- Increased embedding dimensions (128 vs 32)
- Higher lambda_center (0.8 vs 0.1)
- Explicit center initialization at opposite ends of hypersphere
...
LAMBDA_SCL = 0.5  # Weight for supervised contrastive loss
...
def train_model(
    ...
    model_save_path="best_multilabel_gdn_triplet_center.pt",
):
    """
    ...
    # Initialize Triplet-Center Loss
    triplet_center_loss = TripletCenterLoss(
        embed_dim=HIDDEN_DIM,  # Use hidden_dim for embedding dimension
        num_classes=2,
        margin=TCL_MARGIN,
        lambda_c=TCL_LAMBDA_C,
    ).to(device)

    # Initialize Supervised Contrastive Loss
    scl_loss = SupervisedContrastiveLoss(temperature=SCL_TEMPERATURE).to(device)

    # Initialize centers at opposite ends of hypersphere
    with torch.no_grad():
        triplet_center_loss.centers[0] = F.normalize(torch.ones(HIDDEN_DIM), dim=0)
    ...
    for epoch in range(num_epochs):
            ...
            for X_batch, _, sensor_labels_batch, window_labels_batch in pbar:
                ...
                if use_metric_losses:
                    torch.nn.utils.clip_grad_norm_(
                        triplet_center_loss.parameters(), max_norm=0.5
                ...
                if use_metric_losses:
                    train_loss_tcl += loss_tcl.item() * X_batch.size(0)
                    train_loss_scl += loss_scl.item() * X_batch.size(0)
        ...
        center_separation = triplet_center_loss.get_center_separation()
        ...
        with torch.no_grad():
            ...
            for X_batch, _, _, window_labels_batch in val_loader:
                ...
                normal_center = triplet_center_loss.centers[0]
...
def main():
    ...
    # Train model
    model, triplet_center_loss = train_model(
        train_loader,
        val_loader,
