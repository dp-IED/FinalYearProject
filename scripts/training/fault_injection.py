...
def inject_sensor_specific_fault(win, sensor_idx, sensor_name, pid_idx, window_size):
    ...
    if "VEHICLE_SPEED ()" in sensor_name:
        ...
        if win[:, sensor_idx].mean() > 0.30:
            ...
            if np.random.random() > 0.5:
                ...
                win[start:end, sensor_idx] = np.clip(
                    win[start:end, sensor_idx] * 1.3, 0.0, 1.0
                )
            else:
                # Dropout
        ...
        # Engine load drift - gradual decrease
        drift_factor = np.random.uniform(0.7, 0.85)
        ...
        # Long-term fuel trim drift
        drift = np.random.uniform(0.15, 0.25)
