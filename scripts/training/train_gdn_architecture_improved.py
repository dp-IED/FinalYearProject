#!/usr/bin/env python3
"""
Train ImprovedMultiLabelGDN with architectural improvements.

Key improvements:
1. Efficient batched GAT processing (5-10x faster)
2. LayerNorm for training stability
3. GATv2Conv for better attention
4. Multi-layer GAT with residual connections
5. Attention-based aggregation
6. Bidirectional GRU with sensor-specific encoders

Usage:
    python train_gdn_architecture_improved.py --data-path data/carOBD/obdiidata --output anomaly-detection/best_multilabel_gdn_architecture_improved.pt
"""
...
from models.gdn_model import ImprovedMultiLabelGDN, FastImprovedMultiLabelGDN
...
def train_gdn_architecture_improved(
    train_loader,
    val_loader,
    num_sensors,
    window_size,
    ...
    model_save_path="best_multilabel_gdn_architecture_improved.pt",
):
    """
    Train ImprovedMultiLabelGDN with architectural improvements.
    ...
    """
    # Initialize fast improved model (simplified for speed)
    model = FastImprovedMultiLabelGDN(
    ...
    normal_center = nn.Parameter(torch.randn(hidden_dim, device=device) * 0.1)
    ...
    sensor_criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight_value]).to(device))
    global_criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight_value]).to(device))  # Use logits for global too
    ...
    print(f"\n{'='*80}")
    print("Training Improved Multi-Label GDN (Architecture Improvements)")
    print(f"{'='*80}")
    print(f"Configuration:")
    print(f"  Model: ImprovedMultiLabelGDN")
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", leave=False) as pbar:
            for X_batch, _, sensor_labels_batch, window_labels_batch in pbar:
                ...
                # Forward pass - get logits directly (more efficient)
                sensor_logits, global_logits, embeddings = model.forward_logits(X_batch)

                # Classification losses (use logits directly - more stable)
        ...
        with torch.no_grad():
            for X_batch, _, sensor_labels_batch, window_labels_batch in val_loader:
                ...
                sensor_logits = torch.log(sensor_probs_clamped / (1 - sensor_probs_clamped))
                loss_sensor = sensor_criterion(sensor_logits, sensor_labels_batch)
        ...
        anomalous_mean = anomalous_distances.mean() if len(anomalous_distances) > 0 else 0.0
    ...
    if os.path.exists(model_save_path):
        ...
        print(f"  Final Separation Ratio: {checkpoint.get('final_separation_ratio', 0.0):.2f}×")
    else:
        print(f"\n⚠ Warning: No model checkpoint saved (model never improved)")
        print(f"  Best Val F1: {best_val_f1:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Train ImprovedMultiLabelGDN with architectural improvements")
    parser.add_argument("--data-path", type=str, default="data/carOBD/obdiidata", help="Path to data directory")
    parser.add_argument("--output", type=str, default="anomaly-detection/best_multilabel_gdn_architecture_improved.pt", help="Output model path")
    ...
    parser.add_argument("--lambda-separation", type=float, default=1.0, help="Separation loss weight")
    parser.add_argument("--lambda-global", type=float, default=0.3, help="Global loss weight")
    ...
    parser.add_argument("--fault-percentage", type=float, default=0.15, help="Fault injection percentage")
    ...
    data = mean_fill_missing_timestamps_and_remove_duplicates(data, TIME_COL, id_cols=[ID_COL])
    ...
    X_train_sensor, _, train_sensor_labels, train_window_labels = inject_faults_with_sensor_labels(
        X_train, y_train, SENSOR_COLS, fault_percentage=args.fault_percentage, random_state=42
    )
    X_val_sensor, _, val_sensor_labels, val_window_labels = inject_faults_with_sensor_labels(
        X_val, y_val, SENSOR_COLS, fault_percentage=args.fault_percentage, random_state=43
    )
    ...
    test_ds = TensorDataset(X_test_sensor, y_test, test_sensor_labels, test_window_labels)
    ...
    # Train model
    model, normal_center, history = train_gdn_architecture_improved(
        train_loader,
        val_loader,
    ...
    print(f"  2. Compare to original: python diagnose_gdn.py --model {args.output}")
